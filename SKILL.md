---
name: daquele-jeito
description: "Workflow for non-trivial execution projects — plan-first with structured questions, visible progress, 4-axis audit, improvement loop, calibrated effort, bug autonomy, subagents. ACTIVATE ONLY when: (1) user invokes via /daquele-jeito or /that-way, OR (2) one of the trigger phrases (case-insensitive) appears as an INVOCATION IMPERATIVE in the prompt — Portuguese: 'faz daquele jeito' / 'faça daquele jeito'; English: 'do it that way' / 'just do it that way' — typically as a final/standalone instruction meaning 'apply this workflow to the request above/below'. Typical position: in the last lines after the user describes the project, or opening a new request. ACTIVATES (PT): 'I want to build a SaaS for X. [long description]. Faz daquele jeito.' (final instruction), 'Faça daquele jeito the project setup', 'Faz daquele jeito: build X'. ACTIVATES (EN): 'I want to build a SaaS for X. [long description]. Do it that way.' (final imperative), 'Just do it that way: build X', 'Do it that way.' (standalone). DOES NOT ACTIVATE: 'use the alternative method, not faz daquele jeito' / 'not that way' (reference to a discussed method), 'don't do it that way' (negation), 'if we do it that way, it takes longer' (conditional), 'you did it that way yesterday' (past tense), 'how does faz daquele jeito work?' / 'what does that way mean?' (meta-question). Rule of thumb: phrase followed by qualifiers (alternative, previous, that we discussed, the other one) or preceded by negation/conditional/past tense = REFERENCE, not invocation. The English phrase 'do it that way' is more common in everyday speech than its Portuguese counterpart — apply disambiguation MORE strictly for English: only activate when it appears as a final standalone imperative, not embedded in prose discussing a method. WHEN IN DOUBT, DO NOT ACTIVATE — user can invoke explicitly via /daquele-jeito or /that-way."
---
