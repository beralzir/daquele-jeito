---
name: daquele-jeito
description: "Workflow for non-trivial execution projects — plan-first with structured questions, visible progress, 4-axis audit, improvement loop, calibrated effort, bug autonomy, subagents. ACTIVATE ONLY when: (1) user invokes via /daquele-jeito or /that-way, OR (2) the phrase 'faz daquele jeito' / 'faça daquele jeito' (case-insensitive) appears as an INVOCATION IMPERATIVE in the prompt — typically as a final/standalone instruction, meaning 'apply this workflow to the request above/below'. Typical position: in the last lines after the user describes the project, or opening a new request. ACTIVATES: 'I want to build a SaaS for X. [long description]. Faz daquele jeito.' (final instruction), 'Faça daquele jeito the project setup', 'Faz daquele jeito: build X'. DOES NOT ACTIVATE: 'faz daquele jeito alternativo que mencionei' (reference to a discussed method), 'não faz daquele jeito' (negation), 'se fizer daquele jeito...' (conditional), 'você fez daquele jeito ontem' (past tense), 'como funciona o faz daquele jeito?' (meta-question). Rule of thumb: phrase followed by qualifiers (alternativo, anterior, que [verbo], daquele outro) or preceded by negation/conditional/past tense = REFERENCE, not invocation. Note: the trigger phrase is Portuguese on purpose — a literal English version ('do it that way') would be too common in everyday speech and trigger false activations. English-speaking users invoke via the /that-way slash command, which is unambiguous. WHEN IN DOUBT, DO NOT ACTIVATE — user can invoke explicitly via /daquele-jeito or /that-way."
---
